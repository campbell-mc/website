...
export function ScreenshotShowcase() {
  ...
  return (
    <>
      {/* Lightbox */}
      {zoomed && (
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4" onClick={() => setZoomed(null)}>
          <button className="absolute top-4 right-4 p-2 rounded-lg hover:bg-white/10 transition-colors" style={{ color: "rgba(255,255,255,0.6)" }}>
            <X className="w-6 h-6" />
          </button>
          <Image src={zoomed} alt="Platform screenshot" width={1400} height={1050} className="max-w-full max-h-[85vh] object-contain rounded-xl" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
      ...
      <section className="bg-[#faf7f2] border-y border-[#1B4332]/8">
        <div className="max-w-6xl mx-auto px-6 lg:px-16 py-16 lg:py-24">
          ...
          {/* Large stacked screenshots */}
          <div className="space-y-12">
            {screenshots.map((shot) => (
              <div key={shot.title} className="group cursor-pointer" onClick={() => setZoomed(shot.src)}>
                <div
                  className="rounded-2xl overflow-hidden shadow-[0_24px_64px_rgba(0,0,0,0.10),0_0_0_1px_rgba(0,0,0,0.05)] transition-all duration-300 group-hover:shadow-[0_40px_80px_rgba(0,0,0,0.15)] group-hover:-translate-y-1 max-w-4xl mx-auto"
                ...
                <div className="text-center mt-5 max-w-lg mx-auto">
                  <p className="text-[14px] text-stone-600">
                    <span className="font-semibold text-[#1B4332]">{shot.title}.</span> {shot.caption}
                  </p>
                  <p className="text-[11px] text-stone-400 mt-1">Click to enlarge</p>
                </div>
              </div>
            ))}
          </div>
